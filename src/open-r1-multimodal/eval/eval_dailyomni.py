# -*- coding: utf-8 -*-
"""
eval_idea3_reviser_7b.py — Multi-agent pipeline with single Qwen2.5-Omni-7B for Planner/Reviser
Pipeline: Planner -> GRPO_Grounder -> HumanOmniV2 (REINFORCE) -> Reviser
Key optimization: Reuse single Qwen2.5-Omni instance for two roles with LoRA switching
"""
...
Please directly analyze the provided video and audio together with the user query, then output your planning decision as a JSON array."""
...
def stage3_grounder(grounder_proc, video_path, query, duration):
    ...
    if not resp_line: return [[0, duration]], False
...
def main():
    ...
    def get_sample_id(item):
        """用完整 Question + video_id 组合作为唯一标识"""
        q = item.get("Question", "")
        vid = item.get("video_id", "")
        return f"{q}||{vid}"

    def get_short_id(item):
        """生成短且文件系统安全的标识符（用于 adapter_name、LoRA 保存目录）"""
    ...
    for sample in samples_to_process:
        ...
        (use_grounder, gnd_query), planner_raw = stage2_planner(base_model.thinker, base_processor, video_path, query, args.main_gpu)
        ...
        # optimizer = torch.optim.AdamW([p for p in humanomni_model.parameters() if p.requires_grad], lr=args.lr)
        trainable_params = [
