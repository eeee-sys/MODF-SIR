# -*- coding: utf-8 -*-
"""
eval_idea3_dailyomni_human+reviser+no_grpo_grounder.py
Daily-omni ablation pipeline for VideoMind Grounder + HumanOmni + Reviser.
Pipeline: VideoMind Grounder -> HumanOmniV2 (REINFORCE) -> Reviser
"""
...
def main():
    ...
    parser.add_argument(
        "--grpo_adapter_path",
        required=True,
        help="Accepted for CLI compatibility only; ignored by the native grounder worker",
    )
    ...
    parser.add_argument(
        "--base_model_prefix",
        default="",
        help="Optional prefix used by the native grounder worker to resolve relative base_model_path",
    )
    ...
    print("[INFO] Running ablation pipeline: VideoMind Grounder -> HumanOmniV2 -> Reviser")
    ignored_args = {
    ...
    print(f"[INIT] Starting native Grounder process on {args.grounder_gpu}...")
    grounder_script = os.path.join(SCRIPT_DIR, "grounder_worker.py")
    ...
    if args.base_model_prefix:
        grounder_cmd.extend(["--base_model_prefix", args.base_model_prefix])
    ...
    try:
        for sample in samples_to_process:
            ...
            try:
                ...
                stage = "grounder"
                grounder_result = stage3_grounder(grounder_proc, video_path, query, duration)
                ...
                if not grounder_success:
                    raise SampleProcessingError(
                        ...
                        error_type=grounder_result.get("error_type") or "GrounderError",
                ...
                print(f"[Sample Failure] {dataset_id} failed at stage={error_stage}: {exc}")
                failure_entry = {
                    "id": dataset_id,
                    "Question": sample.get("Question"),
                    "video_id": sample.get("video_id"),
                ...
                if error_stage == "grounder" and grounder_proc.poll() is not None:
