...
def append_skipped_sample_log(dataset_id, query, video_path, error_type, error_message, traceback_text, chunk_idx):
    ...
    log_path = os.path.join(log_dir, "skipped_sample_worldsense.log")
    log_entry = {
        "id": dataset_id,
        "query": query,
        "video": video_path,
...
def flatten_worldsense_dataset(dataset_obj, video_root, video_ext):
    """Convert WorldSense nested dict into a flat list of task-level samples."""
    flat_samples = []

    if isinstance(dataset_obj, list):
        return dataset_obj
    if not isinstance(dataset_obj, dict):
        raise ValueError("WorldSense input must be a dict or a list.")

    for outer_video_id, video_item in dataset_obj.items():
        if not isinstance(video_item, dict):
            continue

        video_id = str(video_item.get("video_id") or outer_video_id)
        task_keys = sorted(
            [key for key in video_item.keys() if re.fullmatch(r"task\d+", key)],
            key=lambda key: int(key[4:]),
        )

        for task_key in task_keys:
            task_item = video_item.get(task_key, {})
            if not isinstance(task_item, dict):
                continue

            flat_samples.append({
                "id": f"{video_id}||{task_key}",
                "video_id": video_id,
                "task_key": task_key,
                "video": os.path.join(video_root, f"{video_id}{video_ext}") if video_root else f"{video_id}{video_ext}",
                "data_type": "video",
                "problem": task_item.get("question", ""),
                "options": task_item.get("candidates", []),
                "solution": task_item.get("answer", ""),
                "problem_type": "multiple choice",
                "task_domain": task_item.get("task_domain"),
                "task_type": task_item.get("task_type"),
                "domain": video_item.get("domain"),
                "sub_category": video_item.get("sub_category"),
                "audio_class": video_item.get("audio_class"),
                "video_caption": video_item.get("video_caption"),
                "video_duration_text": video_item.get("video_duration"),
                "duration_bucket": video_item.get("duration"),
            })
...
def get_sample_id(item):
    ...
    return f"{video_id}||{task_key}" if video_id or task_key else "unknown"

def get_short_id(item):
    return hashlib.md5(get_sample_id(item).encode("utf-8")).hexdigest()[:12]

def resolve_video_path(sample, video_root, video_ext):
    video_path = sample.get("video") or sample.get("path")
    if video_path and os.path.exists(video_path):
        return video_path

    video_id = sample.get("video_id", "")
    if video_root and video_id:
        candidate = os.path.join(video_root, f"{video_id}{video_ext}")
...
def main():
    ...
    parser.add_argument("--video_ext", default=".mp4", help="Video filename extension under video_root")
    ...
    with open(args.input_file, 'r', encoding='utf-8') as f:
        dataset = flatten_worldsense_dataset(json.load(f), args.video_root, args.video_ext)
            ...
            for line in f:
                if line.strip():
                    try:
                        ...
                        rec_id = record.get("id") or get_sample_id(record)
                        if rec_id != "unknown":
    ...
    for sample in samples_to_process:
        ...
        video_path = resolve_video_path(sample, args.video_root, args.video_ext)
        trim_path = None
        optimizer = None
        adapter_name = None
        ...
        try:
            if not video_path or not os.path.exists(video_path):
                ...
                error_message = f"Video not found for sample {dataset_id}: {video_path}"
            ...
            if use_grounder:
                ...
                trim_path = os.path.join(tmp_dir, f"trim_{short_id}.mp4")
            ...
            # ====== CLEANUP & PERSIST ======
            sample_lora_dir = os.path.join(args.lora_save_dir, short_id)
