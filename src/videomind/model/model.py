...
class AgentQwen2VLForConditionalGeneration(Qwen2VLForConditionalGeneration):
    ...
    def rescore_candidates(self, video_grid_thw):
        """
        Re-run the timestamp-decoder with gradients enabled, using cached hidden
        states from the most recent training forward pass.

        This method mirrors the 'generating' mode logic (lines 275-356) but:
        - Runs WITH gradients (for GRPO policy optimization)
        - Uses self.cache_norm_state (cached during 'caching' or 'training' mode)
        - Uses the reg_token hidden state from self.model.norm.state

        Returns:
            bnd: [K, 3] tensor of (start_ratio, end_ratio, score) with gradients on score
            Returns None if timestamp-decoder components are not available.
        """
        if not hasattr(self, 'cache_norm_state') or self.cache_norm_state is None:
            return None
        if not hasattr(self, 'cache_vision_inds') or len(self.cache_vision_inds) == 0:
            return None

        # Find the reg_token position in the labels from the last forward pass
        # We use model.norm.state which was cached during the training forward
        if not hasattr(self.model.norm, 'state') or self.model.norm.state is None:
            return None
        ...
        try:
            ...
            # Find reg_token positions in the cached norm state
            # During training forward, norm.state has shape [B, L, hidden_size]
            # We need the hidden state at the reg_token position
            norm_state = self.model.norm.state  # [B, L, hidden_size]

            # Use the last token position as the reg query token
            # (In training mode, the reg_token's hidden state is what drives the decoder)
            if hasattr(self, '_cached_reg_inds') and self._cached_reg_inds is not None:
                reg_inds = self._cached_reg_inds
                reg_tokens = self.reg_proj(norm_state[0, reg_inds, None])
            else:
                # Fallback: use the last position
                reg_tokens = self.reg_proj(norm_state[0:1, -1:])

            # reg_tokens: [num_reg, 1, channel]

            vis_tokens = self.cache_norm_state[0:1, s:e]
            ...
            # vis_tokens: [num_reg, num_frames, channel]
            ...
            # Match training mode: collect all transformer layer outputs
            ...
            # video_emb: [num_reg, num_frames, channel]

            b, t, c = video_emb.size()
            video_msk = video_emb.new_ones(b, t)

            # Padding for short videos (same as training mode)
            ...
            pymid = self.pyramid(pymid_emb, pymid_msk)
            ...
            return bnd[:100]  # [K, 3] with gradients on scores

        except Exception as ex:
            warnings.warn(f'rescore_candidates failed: {ex}')
            return None
